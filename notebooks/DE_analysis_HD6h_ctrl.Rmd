...
7. GO (Gene Ontology) and Plant Reactome Analysis
...
```
```{r}
barplot(d$samples$lib.size * 1e-6, names=1:14, ylab="Library Size (millions)")
```
```{r, results='hide', message=FALSE, warning=FALSE}
d <- estimateDisp(d)
```
...
abline(v=c(-5, 5), col="blue", lwd=3, lty=2)
...
ggplot(aes(x=name_1006, y=n, fill=name_1006, alpha=0.5)) + 
  geom_bar(stat = "identity", width=0.4) + coord_flip() +  labs(title = "Top 10 Cellular Component GO Terms", y = "# of Downregulated Genes in HD6h Wheat", x = "") + theme(legend.position="none", plot.title=element_text(hjust=0.5))
