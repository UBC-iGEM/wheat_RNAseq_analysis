...
output:
  html_document:
    df_print: paged
---
### Exploratory Transcriptome Analysis of Heat and Drought-Stressed Wheat
<br>
...
```{r}
samp_cor <- cor(data)
# heatmap w/ dendrogram to see correlations b/w samples
pheatmap(samp_cor, main="Comparing Heat-Stress, Drought-Stress, Combined-Stress, and Control Sample Conditions", fontsize=9)
```
```{r, results='hide', message=FALSE, warning=FALSE}
...
x_axis <- paste("PC1 (", as.character(round(summary$var_explained[1] * 100, 0)), "%)", sep='')
y_axis <- paste("PC2 (", as.character(round(summary$var_explained[2] * 100, 0)), "%)", sep='')

scores %>% 
  ggplot(aes(x = PC1, y = PC2, colour=condition_group)) +
  geom_point(size = 3, shape=1, stroke=2) +
  labs(x=x_axis, y=y_axis, title = "Principal Component Analysis (PCA)") +
...
z_axis <- paste("PC3 (", as.character(round(summary$var_explained[3] * 100, 0)), "%)", sep='')

plot3d( 
  x=scores$PC1, y=scores$PC2, z=scores$PC3,
  size = 10, xlab=x_axis, ylab=y_axis, zlab=z_axis, col=scores$color_by)
